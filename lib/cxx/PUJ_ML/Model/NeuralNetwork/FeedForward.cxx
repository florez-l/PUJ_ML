    ...
    namespace NeuralNetwork
    {
      ...
      public:
        ...
        virtual void prepare_auxiliary_buffer( const TNatural& M ) const override
          {
            this->free_auxiliary_buffer( );
            this->_prepare_buffers( &( this->m_BufferA ), &( this->m_BufferZ ), nullptr, nullptr, M );
          }
        ...
        TReal cost(
          ...
          {
            return( this->_cost( this->operator()( bX ), bY ) );
          }
      ...
      protected:
        void _prepare_buffers(
          ...
          ) const
          {
            ...
            NZ *= M;
            if( *bufferA == nullptr )
              *bufferA = reinterpret_cast< TReal* >( std::calloc( NA, sizeof( TReal ) ) );
            if( *bufferZ == nullptr )
              *bufferZ = reinterpret_cast< TReal* >( std::calloc( NZ, sizeof( TReal ) ) );
        ...
        void _eval( std::vector< TMatrixMap >& A, std::vector< TMatrixMap >& Z ) const
          {
            TNatural L = this->number_of_layers( );
            for( TNatural l = 1; l <= L; ++l )
            {
              Z[ l - 1 ] = ( A[ l - 1 ] * this->m_W[ l - 1 ] ).rowwise( ) + this->m_B[ l - 1 ];
